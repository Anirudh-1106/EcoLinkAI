...
def get_material_distribution(
    ...
    """Get waste listing distribution by material category, optionally scoped to a company."""
    query = db.query(
    ...
    if company_id:
        query = query.join(Plant, WasteListing.plant_id == Plant.id).filter(
            Plant.company_id == company_id
        )

    rows = (
        query.group_by(Material.material_category)
...
def get_company_analytics(db: Session, company_id: uuid.UUID) -> PlatformAnalytics:
    ...
    total_plants = len(plant_ids)
    total_materials = (
        db.query(func.count(func.distinct(WasteListing.material_id)))
        .filter(WasteListing.plant_id.in_(plant_ids))
        .scalar()
        or 0
    )
    total_listings = (
    ...
    total_requirements = (
        db.query(func.count(Requirement.id))
        .filter(Requirement.plant_id.in_(plant_ids))
        .scalar()
        or 0
    )

    request_scope = (ExchangeRequest.supplier_plant_id.in_(plant_ids)) | (
        ExchangeRequest.buyer_plant_id.in_(plant_ids)
    )
    total_requests = (
        db.query(func.count(ExchangeRequest.id)).filter(request_scope).scalar() or 0
    )

    exchange_scope_query = db.query(Exchange).join(
        ExchangeRequest, Exchange.exchange_request_id == ExchangeRequest.id
    ).filter(request_scope)

    total_exchanges = exchange_scope_query.count()
    ...
    waste_exchanged = (
        db.query(func.coalesce(func.sum(Exchange.actual_quantity), 0))
        .join(ExchangeRequest, Exchange.exchange_request_id == ExchangeRequest.id)
        .filter(request_scope, Exchange.exchange_status == ExchangeStatus.COMPLETED)
        .scalar()
        or Decimal("0.00")
    )
    ...
    accepted = (
        db.query(func.count(ExchangeRequest.id))
        .filter(request_scope, ExchangeRequest.status == ExchangeRequestStatus.ACCEPTED)
        .scalar()
        or 0
    )
    success_rate = (accepted / total_requests * 100) if total_requests > 0 else 0
